...
* Model 1: `Y` predicted by `X` and `M`.
* Model 2: `M` predicted by `X`.
...
dagify(Y ~ X + M, M ~ X, Y ~ M, coords = coords) |> 
    tidy_dagitty() |> 
    mutate(edge = case_when(
        name == "X" & to == "M" ~ get_coef('a', m2, 2),
        name == "M" & to == "Y" ~ get_coef('b', m1, 3),
        name == "X" & to == "Y" ~ get_coef("c'", m1, 2),
        TRUE ~ NA_character_
    )) |>
    mutate(vjust = if_else(name == 'X' & to == 'Y', 2, 0.5)) |>
    ggdag() + theme_dag() +
...
```{r, echo=F, results=F}
a <- summary(m2)$coef[2,1] |> round(3)
b <- summary(m1)$coef[3,1] |> round(3)
```
...
sb <- summary(m1)$coef[3,2]   # SE of b
```
